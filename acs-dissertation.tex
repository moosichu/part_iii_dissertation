...
written in two related languages which are each designed to target different
architectures. The first language, called \textit{the host language}, is a
language that is designed to be compiled for the CPU. The second language,
called the \textit{the shader language} is similarly designed to target GPUs.
...
\end{lstlisting}
\caption{Code written in the pair of new languages that has the same
...
derived from the host language that is being used.
