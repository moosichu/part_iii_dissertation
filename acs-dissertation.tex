...
used language-pairings for graphics programming, this was done to show how a
lot of existing code could stand to benefit from these techniques.
...
architectures. The first language, called \textit{the host language}, is one
designed to be compiled for the CPU. The second language, called the
\textit{the shader language} is similarly designed to target GPUs.
...
of that interface when compiling the shader. We are aiming show the general
benefits of programming in a shader language that is derived from the host
language that is being used, and not any particular features of these
languages.
...
We have presented the two systems we created and have given the motivation for
