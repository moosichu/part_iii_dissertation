...
\begin{itemize}
    ...
    \item Explicit parameter naming in initialisations TODO(Content!).
...
of the type \texttt{Fish}. \texttt{Fish} is a struct whose definition is given
in Listing \ref{lst:struct_fish}.
...
% \label{fig:copy_fish}
% \end{figure}
% TODO(Content!) Workout why this won't compile.
...
process for different graphics toolchains.

\begin{figure}[h]
\centering
\def\svgwidth{0.8\linewidth}
\input{images/pipeline_basic.pdf_tex}
\caption{Compiling a basic struct example. TODO(Content!)}
