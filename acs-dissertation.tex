...
and elaborated on in Section \ref{sec:api_challanges}. It is easy to introduce
bugs due to errors in using the API, which can be alleviate by compile-time
...
API calls (thereby reducing the possibility of errors in using them as the
compiler generates API calls for the programmer), we can acknowledge that they
are different so have host code and shaders written in different programming
languages. However, we can use compile-time type-checking \textit{between}
these languages and compile-time checks on the API calls \textit{themselves} in
order to have the compiler catch errors which would otherwise be left
undetected.

In conclusion, any solution that aims to improve upon current programming
models should give the benefits described above without sacrificing anything
...
The first system is a pre-processor for C and GLSL, that operates on operates
on annotated sections of code. The goal here was to demonstrate how an existing
pair of languages with similar type systems could be extended in order to have
...
could work. The second goal, however, was to show how existing high-level
languages could be extended with a shader-language. This would be suitable for
a language such as Rust, which has a strong type system, but is compromised and
arguably \textit{less safe} than C when using shaders. This is because C and
GLSL can share code as they have similar syntax. Rust and GLSL do not have
similar syntax, meaning that the problem shown in Listing
...
the running of a shader named \texttt{main}. In this example, the host code
makes the \texttt{vkCreateComputePipelines} API call in order to create a
compute pipeline\footnote{This is just an API call that needs to be made TODO:
...
The annotation provides two key features in the particular example. The first
...
ensuring that \texttt{main} cannot renamed in the host or shader without being
consistently renamed in the other. If we were to ensure such an error was
...
\begin{itemize}
    ...
    would be what would eventually run shaders \cite{GeForce256}.
    ...
    the time didn't \cite{GLSL_1_10}.
...
of the market \cite{GPUCrypto} \cite{BitcoinRuiningPricing}. However, there are
many different standards and APIs for different use-cases of the same hardware
as a result of a complex history. Furthermore, many of these use-cases have
...
As can be seen, there has been progress on making programming for heterogeneous
...
from what is currently done both in industry and reasearch. For example, whilst
...
abstracting away the underlying OpenCL API and outputting OpenCL C code for
shaders. However, OpenCL C++ has demonstrated that API interfaces can be
preservered whilst shaders are written in the host language
