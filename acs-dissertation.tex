...
sacrificing the performance that C and GLSL provide. We additionally
demonstrate the power of semantically sharing code can provide, compared with
...
that allows them to share code \textit{modules}.
...
games. Following that, a breakdown of graphics APIs is provided, to give both
...
%TODO(Content!):Reorganise and shrink
...
\caption{TODO(Content!) \cite{UnityShaderPipeline}}.
...
two. We elaborate on this in Section \ref{subsec:code_sharing}.
...
examples, we did not concern ourselves with proving our type-system was sound
or that CUG provided an safety guarantees. CUG is a proof-of-concept language
...
undertaking with over \compilerloccount lines of code. Although CUG-C and
CUG-G share a front-end, their backends of the compiler differ so therefore
we limited the scope of the dialects in the following ways:
...
A big benefit of CUG is the cross-language type-system, and the reason we
developed it after creating AnnoCheck. This different significantly from many
GPU programming models, where the host language and the shader language differ
significantly. Although when using C or C++, existing shader languages are
syntactically similar enough to them for some code such as data structures to
be shared. However, even sharing code like this is limited, as C has a fairly
limited type system. Furthermore, there are no mechanisms for expressing within
files that they are intended to be used for both shader and host code.
...
languages. Therefore, the CUG compiler can compile files marked in this way
whilst being aware of this fact, meaning that when shared modules are included
by any code, the CUG compiler can emit errors if those modules do not conform
to the restricted subset of features which are available in both the host and
shader languages.
...
We round of the chapter with the following figures, which demonstrate the
differences between the different approaches to graphics programming, and how
our methods compare.
...
\caption{Compiling a basic struct example.}
...
The CUG compiler is a much more complex endeavor than AnnoCheck,
...
% as} performant as C - the CUG compiler may not optimise as well as it should.
...
%TODO(Content!) Rewrite this so that comments on the related work is made
